...
// 🔐 Mapbox token (public) - provided via Vite env vars
...
const Map = () => {
  ...
  const RESERVATIONS_API_BASE_URL = ENV.RESERVATIONS_API_BASE_URL;
    ...
    const loadLibs = async () => {
      try {
        ...
        setTimeout(() => {
          if (window.mapboxgl) {
            if (!MAPBOX_TOKEN) {
              console.error("Missing VITE_MAPBOX_TOKEN. Map cannot be initialized.");
              return;
            }
  ...
  const fetchSlotStatuses = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${RESERVATIONS_API_BASE_URL}/zones/${zone.id || zone._id}/slots-status`
      );
      if (!res.ok) return;
  ...
  }, [zone, RESERVATIONS_API_BASE_URL]);
  ...
  const handleReservation = async () => {
    ...
    try {
      ...
      const res = await fetch(`${RESERVATIONS_API_BASE_URL}/reserve`, {
        method: "POST",
      ...
      alert(`Network error: Unable to reach reservations backend (${RESERVATIONS_API_BASE_URL}).`);
      console.error(err);
    } finally {
      setLoadingBook(false);
    }
